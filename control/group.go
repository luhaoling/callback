...
func CallbackAfterSendSingleMsgCommand(c *gin.Context) {
	var req model.CallbackBeforeSendSingleMsgReq
	if err := c.BindJSON(&req); err != nil {
		fmt.Printf("err:%v", err)
		return
	}
	fmt.Printf("CallbackAfterSendSingleMsgCommand received:%#\n", req)
	resp := &model.CallbackBeforeSendSingleMsgResp{
		CommonCallbackResp: model.CommonCallbackResp{
			ActionCode: 0,
			ErrCode:    200,
	...
	fmt.Printf("CallbackAfterSendSingleMsgCommand return%#v\n", resp)
	c.JSON(http.StatusOK, resp)
}
